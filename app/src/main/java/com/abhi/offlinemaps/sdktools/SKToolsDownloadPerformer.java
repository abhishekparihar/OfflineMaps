...
public class SKToolsDownloadPerformer extends Thread {
    ...
    /**
     * Removes current download step is package is not found
     */
    private void deleteCurrentDownloadItem(){
        String path = currentDownloadItem.getCurrentDownloadStep().getDownloadURL();
        if(path.contains(".txg") || path.contains(".ngi")){
            currentDownloadItem.getCurrentDownloadStep().setDownloadItemSize(0);
            try {
                finishCurrentDownload();
    ...
    private long sendAlreadyDownloadedBytes() {
        long bytesRead;
        try {
            final RandomAccessFile destinationFile = new RandomAccessFile(currentDownloadItem.getCurrentStepDestinationPath(), "rw");
    ...
    private void finishCurrentDownload() throws SocketException {
        if (currentDownloadItem != null) {
            ...
            long totalBytesRead;
            try {
                RandomAccessFile currentDestinationFile = new RandomAccessFile(currentDownloadItem.getCurrentStepDestinationPath(), "rw");
            ...
            if (totalBytesRead < currentDownloadItem.getCurrentDownloadStep().getDownloadItemSize()) {
                ...
                if (currentDownloadItem.isDownloadFinished()) {
                    ...
                    if (currentDownloadItem.unzipIsNeeded()) { // UNZIP is needed for current resource
                       ...
                       // Log.d(TAG,"RootDevice" +rootFilePath);
                        Log.d(TAG, "Storage" +rootFilePath);
